...
function getComment($id_actor) {
    ...
    $listComment = $db->prepare('SELECT gbaf_comment.comment, DATE_FORMAT(date_add, \'%d/%m/%Y à %Hh%imin%ss\') AS date_add_fr, gbaf_member.firstname, gbaf_member.id FROM gbaf_comment
        LEFT JOIN gbaf_member ON gbaf_comment.gbaf_member_id = gbaf_member.id
        WHERE gbaf_comment.gbaf_actor_id = ? ORDER BY gbaf_comment.date_add DESC');
...
function getVoteByActor($îd_actor) {
    require('includes/connexion.php');
    $listVotes = $db->prepare('SELECT * FROM gbaf_vote WHERE gbaf_actor_id= ?');
...
function getLikeByActor($id_actor) {
    require('includes/connexion.php');
    $req = $db->prepare('SELECT COUNT(gbaf_vote.vote) AS nb_vote FROM gbaf_vote WHERE gbaf_actor_id = ? AND vote = 1');
...
function getDislikeByActor($id_actor) {
    require('includes/connexion.php');
    $req = $db->prepare('SELECT COUNT(gbaf_vote.vote) AS nb_vote FROM gbaf_vote WHERE gbaf_actor_id = ? AND vote = 0');
    $req->execute(array($id_actor));
    $dislike = $req->fetch();
    return $dislike;
}
...
function getLikes() {
    require('includes/connexion.php');
    $listLikes = $db->query('SELECT COUNT(*) AS nb_likes, gbaf_actor_id FROM gbaf_vote WHERE vote = 1 GROUP BY gbaf_actor_id');
...
function getDislikes() {
    require('includes/connexion.php');
    $listDislikes = $db->query('SELECT COUNT(*) AS nb_dislikes, gbaf_actor_id FROM gbaf_vote WHERE vote = 0 GROUP BY gbaf_actor_id');
...
function getCommentExist($id_actor, $id_member) {
    ...
    $req = $db->prepare('SELECT gbaf_comment.comment FROM gbaf_comment
        LEFT JOIN gbaf_member ON gbaf_comment.gbaf_member_id = gbaf_member.id
        WHERE gbaf_comment.gbaf_actor_id = ? AND gbaf_comment.gbaf_member_id = ?');
...
function getVoteExist($id_actor, $id_member) {
    ...
    $req = $db->prepare('SELECT gbaf_vote.vote FROM gbaf_vote
        LEFT JOIN gbaf_member ON gbaf_vote.gbaf_member_id = gbaf_member.id
        WHERE gbaf_vote.gbaf_actor_id = ? AND gbaf_vote.gbaf_member_id = ?');
    $req->execute(array($id_actor, $id_member));
    $voteExist = $req->fetch();
    return $voteExist;
}
...
function insertComment($comment, $idActor, $idMember) {
    require('includes/connexion.php');
    $req = $db->prepare('INSERT INTO gbaf_comment(comment, date_add, gbaf_actor_id, gbaf_member_id) VALUES(:comment, NOW(), :actor_id, :member_id)');
    $req->execute(array(
        'comment' => $comment,
        'actor_id' => $idActor,
        'member_id' => $idMember
...
function insertLike($vote, $idActor, $idMember) {
    require('includes/connexion.php');
    $req = $db->prepare('INSERT INTO gbaf_vote(vote, gbaf_actor_id, gbaf_member_id) VALUES(:vote, :actor_id, :member_id)');
    $req->execute(array(
        'vote' => $vote,
        'actor_id' => $idActor,
        'member_id' => $idMember
